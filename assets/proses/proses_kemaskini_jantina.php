...
if (!empty($_POST))
{
	...
	$data_gender_name = $_POST['gender_name'];

	#pengesahan data
	if (empty ($data_gender_name)) 
	{
		echo "<script> 
			setTimeout(function() {
				Swal.fire({
		...
		$arahan_sql_kemaskini = "UPDATE lt_gender SET gender_name = '".$data_gender_name."' WHERE id_gender = '".$id_gender."'";
			
		#melaksanakan proses menyimpan data dalam syarat ifgender_name
		if(mysqli_query($conn,$arahan_sql_kemaskini))
		{
			...
			redirect("senarai_jantina.php?success=2");
		}
		else
		{	
